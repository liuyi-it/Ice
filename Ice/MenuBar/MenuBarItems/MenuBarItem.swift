...
struct MenuBarItem {
    ...
    var isMovable: Bool {
        !MenuBarItemInfo.immovableItems.contains(info)
    }

    /// A Boolean value that indicates whether the item can be hidden.
    var canBeHidden: Bool {
        !MenuBarItemInfo.nonHideableItems.contains(info)
    }
...
private extension MenuBarItemInfo {
    ...
    init(uncheckedItemWindow itemWindow: WindowInfo) {
        let bundleIdentifier = itemWindow.owningApplication?.bundleIdentifier
        if
            #available(macOS 26.0, *),
            let bundleIdentifier,
            bundleIdentifier == MenuBarItemInfo.Namespace.controlCenter.rawValue,
            let title = itemWindow.title,
            ControlItem.Identifier.allCases.map(\.rawValue).contains(title)
        {
            // macOS 26 reparents Ice's status items to Control Center.
            // Their titles remain stable, so restore the Ice namespace.
            self.namespace = .ice
        } else if
            #available(macOS 26.0, *),
            let bundleIdentifier,
            bundleIdentifier == MenuBarItemInfo.Namespace.controlCenter.rawValue,
            itemWindow.title == nil || itemWindow.title == "Item-0"
        {
            // macOS 26 reparents third-party status items to Control Center
            // and may omit their titles or replace them with the generic
            // "Item-0". Use a per-window cache identity so views and captured
            // images do not overwrite each other.
            self.namespace = Namespace("\(bundleIdentifier).window.\(itemWindow.windowID)")
