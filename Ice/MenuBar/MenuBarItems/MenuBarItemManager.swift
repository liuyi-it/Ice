...
extension MenuBarItemManager {
    ...
    func cacheItemsIfNeeded(force: Bool = false) async {
        ...
        // Try frame-based matching first (macOS 26+ compatibility),
        // fall back to title-based matching.
        let hiddenControlItem: MenuBarItem?
        if let hiddenSection {
            hiddenControlItem = items.firstIndex(matchingControlItem: hiddenSection.controlItem).map({ items.remove(at: $0) })
                ?? items.firstIndex(matching: .hiddenControlItem).map({ items.remove(at: $0) })
        } else {
        ...
        if let alwaysHiddenSection {
            alwaysHiddenControlItem = items.firstIndex(matchingControlItem: alwaysHiddenSection.controlItem).map({ items.remove(at: $0) })
                ?? items.firstIndex(matching: .alwaysHiddenControlItem).map({ items.remove(at: $0) })
        } else {
        ...
        guard let hiddenControlItem else {
            Logger.itemManager.debug("Hidden control item is not ready, keeping the previous cache and retrying later")
            return
        }
...
extension MenuBarItemManager {
    ...
    private func scrombleEvent(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            eventTap1.enable()
            eventTap2.enable(timeout: .milliseconds(250)) {
...
extension MenuBarItemManager {
    /// Returns the items that should be shown in the Ice Bar for the given section.
    func iceBarItems(for section: MenuBarSection.Name, on screen: NSScreen) -> [MenuBarItem] {
        let sectionItems = itemCache.managedItems(for: section)
        guard section == .hidden else {
            return sectionItems
        }

        let inaccessibleVisibleItems = itemCache.managedItems(for: .visible).filter { item in
            !itemIsDirectlyClickable(item, on: screen)
        }

        return (inaccessibleVisibleItems + sectionItems).uniquedByInfo()
    }

    /// Returns whether a menu bar item can currently receive a direct click.
    func itemIsDirectlyClickable(_ item: MenuBarItem, on screen: NSScreen? = nil) -> Bool {
        let items = MenuBarItem.getMenuBarItems(onScreenOnly: false, activeSpaceOnly: true)
        guard
            let currentItem = currentItem(matching: item, in: items),
            itemIsInVisibleSection(currentItem, in: items),
            currentItem.isOnScreen
        else {
            return false
        }

        let screen = screen ?? NSScreen.screens.first { $0.frame.intersects(currentItem.frame) }
        if let screen {
            guard screen.frame.intersects(currentItem.frame) else {
                return false
            }
            if let notchFrame = screen.frameOfNotch, notchFrame.intersects(currentItem.frame) {
                return false
            }
            if
                let appState,
                let applicationMenuFrame = appState.menuBarManager.getApplicationMenuFrame(for: screen.displayID),
                currentItem.frame.minX < applicationMenuFrame.maxX
            {
                return false
            }
        }
    ...
    /// Returns the current menu bar item matching the given item.
    private func currentItem(matching item: MenuBarItem, in items: [MenuBarItem]) -> MenuBarItem? {
        items.first { $0.windowID == item.windowID }
            ?? items.first { $0.info == item.info }
    }

    /// Returns the hidden control item index, using frame matching before
    /// falling back to the title-based identity.
    private func hiddenControlItemIndex(in items: [MenuBarItem]) -> [MenuBarItem].Index? {
        let hiddenSection = appState?.menuBarManager.section(withName: .hidden)
        return hiddenSection.flatMap { items.firstIndex(matchingControlItem: $0.controlItem) }
            ?? items.firstIndex(matching: .hiddenControlItem)
    }

    /// Returns whether the given item is currently in the visible section.
    private func itemIsInVisibleSection(_ item: MenuBarItem, in items: [MenuBarItem]) -> Bool {
        guard
            let currentItem = currentItem(matching: item, in: items),
            currentItem.isOnScreen,
            let hiddenControlItemIndex = hiddenControlItemIndex(in: items)
        else {
            return false
        }
        return Predicates.isInVisibleSection(hiddenControlItem: items[hiddenControlItemIndex])(currentItem)
    }
    ...
    func tempShowItem(_ item: MenuBarItem, clickWhenFinished: Bool, mouseButton: CGMouseButton) {
        ...
        if itemIsDirectlyClickable(item) {
            if clickWhenFinished {
                let itemToClick = currentItem(matching: item, in: items) ?? item
                Logger.itemManager.info("\(itemToClick.logString) is already visible, clicking directly")
                Task {
                    do {
                        try await click(item: itemToClick, with: mouseButton)
                    } catch {
        ...
        guard let hiddenControlItemIndex = hiddenControlItemIndex(in: items) else {
            Logger.itemManager.warning("No hidden control item, so not showing \(item.logString)")
            return
        }
        items.removeSubrange(...hiddenControlItemIndex)
...
private extension Sequence where Element == MenuBarItem {
    /// Returns the items after removing duplicate item infos.
    func uniquedByInfo() -> [MenuBarItem] {
        var seen = Set<MenuBarItemInfo>()
        return filter { item in
            seen.insert(item.info).inserted
        }
    }
}
