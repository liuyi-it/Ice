...
final class MenuBarItemSpacingManager {
    ...
    /// Maximum time to wait after force terminating an app.
    private let forceTerminateTimeout = 5
    ...
    private func signalAppToQuit(_ app: NSRunningApplication) async throws {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let timeoutTask = Task {
                ...
                try await Task.sleep(for: .seconds(forceTerminateTimeout))
                guard !hasResumed else {
                    return
                }
                hasResumed = true
                cancellable?.cancel()
                continuation.resume(throwing: GroupedRelaunchError(failedApps: [logString(for: app)]))
            }
