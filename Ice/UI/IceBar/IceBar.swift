...
final class IceBarPanel: NSPanel {
    ...
    private func updateOrigin(for screen: NSScreen) {
        ...
        func getOrigin(for iceBarLocation: IceBarLocation) -> CGPoint {
            ...
            func originCentered(at x: CGFloat) -> CGPoint {
                let lowerBound = screen.frame.minX
                let upperBound = screen.frame.maxX - frame.width
                guard lowerBound <= upperBound else {
                    return originForRightOfScreen
                }
                return CGPoint(x: (x - frame.width / 2).clamped(to: lowerBound...upperBound), y: originY)
            }
            ...
            case .mousePointer:
                if let location = MouseCursor.locationAppKit {
                    return originCentered(at: location.x)
                }
                if
                    let section = appState.menuBarManager.section(withName: .visible),
                    let itemFrame = section.controlItem.windowFrame
                {
                    return originCentered(at: itemFrame.midX)
                }
                return originForRightOfScreen
            case .iceIcon:
                if
...
private struct IceBarContentView: View {
    ...
    private var items: [MenuBarItem] {
        itemManager.iceBarItems(for: section, on: screen)
    }
