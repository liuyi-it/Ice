...
extension Collection where Element == MenuBarItem {
    ...
    /// Returns the first index where a menu bar item's frame matches the given
    /// control item's window frame.
    ///
    /// On macOS 26+, Control Center can reparent status item windows, so neither
    /// the title nor owner PID can reliably identify Ice's control items.
    @MainActor
    func firstIndex(matchingControlItem controlItem: ControlItem) -> Index? {
        guard let controlFrame = controlItem.windowFrame else { return nil }

        func trailingEdgeDistance(for item: MenuBarItem) -> CGFloat {
            abs(item.frame.maxX - controlFrame.maxX)
        }

        let index = indices.min {
            trailingEdgeDistance(for: self[$0]) < trailingEdgeDistance(for: self[$1])
        }
        guard
            let index,
            trailingEdgeDistance(for: self[index]) <= 2
        else {
            return nil
        }
        return index
